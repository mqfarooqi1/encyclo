...
export const api = {
  ...
  trails: () => request(`/api/trails${qs(kids())}`),
  trail: (key) => request(`/api/trail/${encodeURIComponent(key)}`),
  recordStation: (trail, station, score, total) => request(
    `/api/trail/${encodeURIComponent(trail)}/${encodeURIComponent(station)}`,
    { method: 'POST', body: JSON.stringify({ score, total }) },
  ),
  badges: () => request('/api/badges'),
  resetTrails: (trail) => request(`/api/progress/trails${qs({ trail })}`, { method: 'DELETE' }),
